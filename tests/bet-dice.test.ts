...
const X402_ENDPOINT = `/api/bet/dice`;
...
export async function testX402ManualFlow(verbose = false) {
  ...
  const logger = createTestLogger("bet-dice", verbose);
  ...
  for (const tokenType of TEST_TOKENS) {
    ...
    if (
      ...
      logger.success(`Dice max50 → roll ${data.roll} (${data.won ? "WIN" : "LOSE"}, ${data.multiplier.toFixed(2)}x) for ${tokenType}`);
