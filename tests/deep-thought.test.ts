...
const X402_ENDPOINT = `/api/deep-thought`;
...
export async function testX402ManualFlow(verbose = false) {
  ...
  const logger = createTestLogger("deep-thought", verbose);
  ...
  for (const tokenType of TEST_TOKENS) {
    ...
    if (thought && typeof thought === 'string' && data.tokenType === tokenType) {
      logger.success(`Thought: "${thought.substring(0, 50)}${thought.length > 50 ? '...' : ''}" for ${tokenType}`);
      tokenResults[tokenType] = true;
    } else {
      logger.error(`Validation failed for ${tokenType}: thought=${!!thought}, token match=${data.tokenType === tokenType}`);
