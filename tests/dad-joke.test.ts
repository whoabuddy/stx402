...
const X402_ENDPOINT = `/api/dad-joke`;
...
export async function testX402ManualFlow(verbose = false) {
  ...
  const logger = createTestLogger("dad-joke", verbose);
  ...
  for (const tokenType of TEST_TOKENS) {
    ...
    if (joke && typeof joke === 'string' && data.tokenType === tokenType) {
      logger.success(`Joke: "${joke.substring(0, 50)}${joke.length > 50 ? '...' : ''}" for ${tokenType}`);
      tokenResults[tokenType] = true;
    } else {
      logger.error(`Validation failed for ${tokenType}: joke=${!!joke}, token match=${data.tokenType === tokenType}`);
